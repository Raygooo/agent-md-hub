    ...
    <Shell>
      <section className="mx-auto grid max-w-7xl gap-8 px-5 pb-20 pt-8 sm:px-6 md:pt-10 lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          ...
          <ol className="mt-5 grid gap-3 text-sm text-slate-300 sm:grid-cols-3 lg:grid-cols-1">
          ...
          <div className="mt-8 space-y-3">
            {apps.map((app) => (
              <div key={app.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-1 break-all text-sm text-slate-400">{app.docs.length} docs · /{app.ownerSlug}/{app.slug}</div>
              </div>
            ))}
          </div>
        </div>
        <form action="/api/publish" method="post" className="glass rounded-[2rem] p-5 sm:p-6">
          {protectedWrites && (
            <label className="mb-4 block text-sm text-slate-300">Admin token<input name="adminToken" type="password" placeholder="Required to publish" className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-white" /></label>
          )}
          ...
          <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Link href="/registry" className="text-center text-sm text-slate-300 hover:text-white">View registry</Link>
            <button disabled={readonly} className="rounded-full bg-white px-6 py-3 font-semibold text-slate-950 hover:bg-cyan-100 disabled:cursor-not-allowed disabled:opacity-50">{readonly ? 'Publishing disabled' : 'Publish doc'}</button>
          </div>
        </form>
      </section>
    </Shell>
...
function Step({ n, text }: { n: string; text: string }) {
  return (
    <li className="flex gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-white text-sm font-semibold text-slate-950">{n}</span>
      <span>{text}</span>
    </li>
  );
}
