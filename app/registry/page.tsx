    ...
    <Shell>
      <section className="mx-auto max-w-7xl px-5 pb-20 pt-8 sm:px-6 md:pt-10">
        <div className="mb-6 flex flex-col justify-between gap-4 md:mb-8 md:flex-row md:items-end">
          ...
          <Link href="/studio" className="rounded-full bg-white px-5 py-3 text-center font-medium text-slate-950 hover:bg-cyan-100">Publish yours</Link>
        </div>
            ...
            <article key={app.id} className="glass rounded-3xl p-5 sm:p-6">
              ...
              <div className="mt-6 space-y-3">
                {app.docs.map((doc) => {
                  ...
                  return (
                    <div key={doc.id} className="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                      <Link href={previewUrl} className="block hover:text-cyan-100">
                        <div className="font-medium">{doc.title}</div>
                        <div className="mt-1 break-all font-mono text-xs text-slate-400">{absoluteSiteUrl(rawUrl)}</div>
                      </Link>
                      <div className="mt-3">
